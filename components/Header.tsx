      ...
      <div>
        <nav className="text-slate-600 dark:text-slate-300">
          <Link href="/entries/new" className="mx-2">
            <button className="font-semibold text-white text-sm bg-gradient-to-r from-indigo-400 via-purple-400 to-cyan-400 hover:from-indigo-500 hover:via-purple-500 hover:to-cyan-500 rounded-md shadow-sm px-3 py-2">
              New Entry
            </button>
          </Link>
          {!session && (
          ...
          {session && (
            <div ref={dropdownMenuRef} className="relative inline-block text-left">
              <button type="button" onClick={() => toggleDropdownMenu()} className="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 dark:text-gray-100 dark:bg-slate-800 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700" id="menu-button" aria-expanded="true" aria-haspopup="true">
                ...
                <div className={`${showDropdownMenu ? "scale-100" : "scale-0 invisible"} transition-all duration-200 ease-out absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white dark:bg-slate-900 text-gray-700 dark:text-gray-100 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none`} role="menu" aria-orientation="vertical" aria-labelledby="menu-button">
