...
const getKey = (pageIndex: number, previousPageData: any) => {
  ...
  if (pageIndex === 0) return `/api/entries?limit=${PAGE_SIZE}`;

  // add the cursor to the API endpoint
  return `/api/entries?limit=${PAGE_SIZE}&cursor=${previousPageData[previousPageData.length - 1].id}`;
}
...
export default function EntriesPage() {
  ...
  return (
    <div>
      <h2 className="mb-10 text-2xl font-black dark:text-slate-100">My Entries</h2>
      {data && data.length > 0 ? (
        <>
